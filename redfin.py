...
class RedFin():
    def __init__(self):
        self.host = "https://www.redfin.com/"
        self.city = 'city/30818/TX/Austin/'
    ...
    def get_search_results(self):
        page_source = self.request_search_page(self.host + self.city)
    ...
    def get_property_data(self, property_urls):
        count = 0
        for property_url in property_urls:
            self.output_data.append(self.get_property_page(self.host + property_url))
            count += 1
    ...
    def make_page_request_no_proxy(self, property_url):
        ...
        for i in range(10):
            try:
                http_response = requests.get(property_url, headers=user_agent_header, verify=False)
    ...
    def parse_property_page(self, page_source, property_url):
        ...
        try:
            property_data['sqFt'] = int(self.soup.find('span', attrs={'data-rf-test-id': 'abp-lotSize'}).find('span', attrs={'class': 'value'}).get_text().replace("Sq. Ft.", "").replace(",", ""))
        except:
        ...
        try:
            timedelta = self.soup.find('div', attrs={"data-rf-test-id": "abp-homeinfo-homebottomstats"}).find('div').find('div').select('span')[8].string
            property_data['listing_start_time'] = datetime.datetime.fromtimestamp(datetime.datetime.now().timestamp() - timeparse(timedelta))
        except:
